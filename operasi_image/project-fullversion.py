...
# Form implementation generated from reading ui file 'project-fullversion.ui'
#
...
from OperasiAritmatika import Ui_MainWindow
...
class Ui_Project1(object):
    ...
    def retranslateUi(self, Project1):
        ...
        Project1.setWindowTitle(_translate("Project1", "Mini Photoshop"))
        ...
        self.actionAverage.triggered.connect(self.convertToGrayscaleAverage)
        self.actionLightness.triggered.connect(self.convertToGrayscaleLightness)
        self.actionLuminance.triggered.connect(self.convertToGrayscaleLuminance)
        self.actionInvers.triggered.connect(self.convertToInvers)
        self.actionVertikal.triggered.connect(self.convertToFlipVertikal)
        self.actionHorizontal.triggered.connect(self.convertToFlipHorizontal)

        self.actionAritmatika.triggered.connect(self.Operasi)
    ...
    def convertToGrayscaleAverage(self):
            ...
            if pixmap:
                ...
                grayscale_pixmap = QPixmap.fromImage(img)
                self.label_2.setPixmap(grayscale_pixmap)

    def convertToGrayscaleLightness(self):
            pixmap = self.label.pixmap()
            if pixmap:
                img = pixmap.toImage()
                for x in range(img.width()):
    ...
    def convertToGrayscaleLuminance(self):
            pixmap = self.label.pixmap()
            if pixmap:
                img = pixmap.toImage()
                for x in range(img.width()):
        ...
        if pixmap:
            ...
            for x in range(img.width()):
                for y in range(img.height()):
                    pixel = img.pixel(x, y)
                    r, g, b = QtGui.qRed(pixel), QtGui.qGreen(pixel), QtGui.qBlue(pixel)
                    inverted_pixel = qRgb(255 - r, 255 - g, 255 - b)
                    img.setPixel(x, y, inverted_pixel)

            inverted_pixmap = QPixmap.fromImage(img)
            self.label_2.setPixmap(inverted_pixmap)

    def convertToFlipVertikal(self):
        pixmap = self.label.pixmap()
        if pixmap:
            img = pixmap.toImage()
            flipped_img = QImage(img.width(), img.height(), QImage.Format_RGB32)

            for x in range(img.width()):
                for y in range(img.height()):
                    pixel = img.pixel(x, y)

                    flipped_y = img.height() - 1 - y

                    flipped_img.setPixel(x, flipped_y, pixel)

            flipped_pixmap = QPixmap.fromImage(flipped_img)
            self.label_2.setPixmap(flipped_pixmap)


    def convertToFlipHorizontal(self):
        pixmap = self.label.pixmap()
        if pixmap:
            img = pixmap.toImage()
            ...
            for x in range(img.width()):
                for y in range(img.height()):
                    ...
                    flipped_x = img.width() - 1 - x
                    
                    flipped_img.setPixel(flipped_x, y, pixel)
